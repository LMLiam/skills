...
`openai-gpt-5.6` and `opencode-gpt-5.6` share identical tier mappings (Terra for breadth, Sol for ambiguity and high consequence). Each other profile is native to its model family and defines its own agent surface: `opencode-claude-5` uses ten tiers across Sonnet 5, Opus 5, and Fable 5; `opencode-gemini-3` uses four tiers across Flash 3.7 and Pro 3.1. The `opencode-mixed` profile is different: it is a flat ladder of thirteen tiers that spans model families, ordered by the Artificial Analysis Intelligence Index with price as the tiebreak, and each tier is named by its model and variant. Do not treat tier names as equivalent to another provider profile. Add another profile only after it has a reviewed mapping and passes the same installation, safety, and behavioural checks.
